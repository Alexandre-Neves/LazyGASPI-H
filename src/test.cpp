...
int main(int argc, char** argv){
    ...
    while((ch = getopt_long(argc, argv, "hk:n:r:s:i:", options, nullptr)) != -1) {
        switch(ch){
    ...
    SUCCESS_OR_DIE_OUT(std::cout, 
        lazygaspi_init(table_amount, table_size, ROW_SIZE,
                      [](LazyGaspiProcessInfo* info){
                        SUCCESS_OR_DIE_OUT(std::cout, gaspi_setup_output("lazygaspi_h", info->id, &(info->out)));
                      }
        )
    );
    #else
