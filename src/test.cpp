...
int main(int argc, char** argv){
    ...
    while((ch = getopt_long(argc, argv, "hk:n:r:s:g:2:", options, nullptr)) != -1) {
        switch(ch){
    ...
    SUCCESS_OR_DIE_COUT(lazygaspi_init(table_amount, table_size, ROW_SIZE,
                            [](LazyGaspiProcessInfo* info){
                                SUCCESS_OR_DIE_COUT(gaspi_setup_output("lazygaspi_h", info->id, &(info->out)));
                                timestamp(*info->out);
                                PRINT_DEBUG("\n\t\t\t//////////////////////\n\t\t\t//\tRANK " << info->id 
                                            << "      //\n\t\t\t//////////////////////\n");
                            }
    ...
    #if defined DEBUG || defined DEBUG_PERF || defined DEBUG_TEST || defined OUTPUT_RESULT
